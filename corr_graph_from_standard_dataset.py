...
folder = 'D:\\simulation\\hajj\\testing\\frames\\02\\'
save_folder = 'graph_save\\hajj\\test02\\'
...
while True:
    ...
    if True:
        ...
        for posi in posis:
            # print(posi,index)
            c, spatial_entropy = local_color_from_render((posi[0], posi[1]), frame, cal_radius, graph_sample=False, return_entropy=True)
        ...
        if node_feature[0]:
            ...
            np.savez(save_folder + '0' * (5 - len(str(frame_index))) + str(frame_index) + '.npz', f=node_feature, a=adj, a2=spatio_adj)
